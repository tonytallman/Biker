...
/// Running maximum of measurement samples while the context's activity is ``ActivityState/active``.
public final class MaximumMetric<U: Dimension>: Metric {
    public typealias UnitType = U

    public let publisher: AnyPublisher<Measurement<U>, Never>

    /// - Parameters:
    ///   - source: Instantaneous samples (e.g. current speed).
